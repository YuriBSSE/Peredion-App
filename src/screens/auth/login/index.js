...
const Login = ({navigation, login}) => {
  ...
  function onLogin() {
    ...
    if (fields.password && fields.email) {
      ...
      // axios.get(`https://12rider.com/api.php?email=${fields.email}&password=${fields.password}`).then( async (res)=>{
       
      //   if(res.status == 200){
      //     console.log(res.data?.user[0]?.user)
      //     await AsyncStorage.setItem('data', JSON.stringify(res.data?.user[0]?.user))
      //     setLoading(false);
      //     Toast.show({
      //       type:"success",
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView
        ...
        <View style={{backgroundColor: '#383838', borderRadius: 12, padding: 10}}>
        <Image
          style={styles.logo}
          resizeMode="contain"
 ...
 ? 1 : 1.6),
        ...
        <View style={styles.btnCont}>
          ...
          <Btn text="Sign Up" call={() => navigation.navigate('signup')} loader={null} />
          {/* <Btn
          text="Sign Up"
          call={() => {
